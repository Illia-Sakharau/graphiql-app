...
describe("Main page", () => {  
  ...
  test("Render demo section", async () => {
    ...
    expect(phrase2p2.innerHTML).toEqual('от широкоэкранных до мобильных устройств');
  });

  test("Render school section", async () => {
    const schoolLinks = screen.getAllByAltText('Link to The Rolling Scopes School');
  ...
  test("Render our team section", async () => {
    ...
    const subTitle = screen.getByText('G.O.D.S. (Gurus Of Developmental Skills)');
  ...
  test("Render herro-banner section", async () => {
    ...
    const subTitle = screen.getByText('GraphiQL is a playground/IDE for graphQL requests.');
    ...
    expect(subTitle.innerHTML).toEqual('GraphiQL - это playground/IDE для graphQL запросов.');
